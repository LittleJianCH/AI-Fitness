...
Current backend targets include build, domain/API checking, tests, formatting/linting, contract generation/tests, and FIT/native checks. The web app currently defines `check`, `lint`, and `build`; it does not yet define a unit/browser test script. Contract generation and TypeScript/Swift consumer checks are documented in `README.md` and `docs/api-contract.md`. Run the smallest relevant configured checks for the changed boundary and report platform-specific checks separately.
