...
| iOS | Swift, SwiftUI, HealthKit; URLSession, Keychain; Swift OpenAPI Generator | SwiftUI login, backend workout list/detail with charts/routes, generated Swift API transport and Keychain sessions implemented in `ios/`. HealthKit selection/preview and explicit import are implemented; platform export remains a subsequent slice. Physical-device acceptance remains separate from synthetic tests. SwiftData is conditional on actual local persistence needs; it does not duplicate the server database. |
