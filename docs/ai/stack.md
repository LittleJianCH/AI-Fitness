...
| Database | PostgreSQL using the selected IHP version's Hasql-based infrastructure | User/session/workout storage and IHP SQL migrations implemented; HTTP handlers use a bounded Hasql pool. |
...
| API contract | Servant-derived OpenAPI 3.1 | Draft implemented, with a tested 3.0-to-3.1 conversion and TypeScript/Swift consumers. Eleven authentication operations are mounted; other groups remain contracts. |
