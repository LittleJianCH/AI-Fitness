...
| iOS | Swift, SwiftUI, HealthKit; URLSession, Keychain; Swift OpenAPI Generator | SwiftUI login application, generated Swift API transport and Keychain sessions implemented in `ios/`. Workout browsing and HealthKit remain subsequent slices. SwiftData is conditional on actual local persistence needs; it does not duplicate the server database. |
