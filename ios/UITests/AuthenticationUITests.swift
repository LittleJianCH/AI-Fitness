...
final class AuthenticationUITests: XCTestCase {
    ...
    func testSettingsPersistBodyParametersAndEquipment() throws {
        ...
        mass.tap()
        mass.typeText("72.5.4")
        app.buttons["saveBodyParameters"].tap()
        XCTAssertTrue(mass.exists, "An invalid numeric suffix must not save or close the editor")
        let invalidNumber = app.staticTexts["数值需要大于零；没有数据时请留空。"]
        reveal(invalidNumber, in: app)
        XCTAssertTrue(invalidNumber.exists)
        capture(app, name: "Invalid body parameter rejected")
        app.buttons["取消"].tap()
        app.buttons["editBodyParameters"].tap()
        XCTAssertTrue(mass.waitForExistence(timeout: 10))
        XCTAssertEqual(mass.value as? String, "未设置", "Rejected input must not persist")
        mass.tap()
        mass.typeText("72.5")
