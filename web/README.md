...
English is the source/fallback language; Simplified Chinese is also supported.
The language picker stores only a validated browser-local cookie. System clears
that override and follows ordered, quality-weighted browser preferences. URLs,
account settings, canonical values and user-authored titles/notes do not change.
Language changes reload the page only after the existing draft protection accepts
the change; active form submissions block the reload.

Maintain complete sentences in `messages/en.json` and `messages/zh-Hans.json`.
Use the typed functions generated by Paraglide directly; do not translate user
content or build keys from wire values. `pnpm i18n:generate` validates both catalogs
and generates ignored modules before prepare/check/test; the Vite plugin handles
dev/build. Both compiler and message-format plugin are pinned in the dependency
lock, and the plugin is loaded locally. `pnpm i18n:check` checks catalog parity,
placeholders, nonempty translations and English plural branches.

Server language uses request-local Paraglide context. Hydration follows document
language, and HTML is private/no-store. Regional formatting uses a matching
browser preference (for example en-GB), separately from message language. Date
group keys use Gregorian dates in the browser's time zone; labels are localized.
Personal date labels are deferred when the browser zone is unavailable in SSR.

Localization regression tests cover negotiation, concurrent SSR, date boundaries,
plural messages, generated runtime fallback for a missing translation, safe switching
