...
export const manualDraftSchema = z
	.object({
		...
		start: z.string().refine((value) => !!localStart(value), '请填写有效的本地开始时间。'),
		minutes: z.number().int().nonnegative({ message: '分钟需要是非负整数。' }),
	...
	.superRefine((draft, context) => {
		...
		if (start && (!end || !Number.isFinite(end.getTime()) || end.getUTCFullYear() > 9999))
			context.addIssue({ code: 'custom', path: ['minutes'], message: '时长超出支持的时间范围。' });
	});
